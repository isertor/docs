...
- Android:
  ...
  Xiaomi Pad Pro or similar specs
- iOS
...
We recommend using business-grade routers like Ubiquiti UniFi Dream Machine UDM-US or ASUS ZenWiFi AX (XT8) for the best performance. These routers are reliable and will not compromise speed when connecting multiple devices.

Choose the router based on your restaurant size — smaller spaces need fewer connections, while larger setups require more. Some of the criteria to consider:
...
If possible, avoid . They tend to have slower speeds and a limited number of supported devices. Low-spec home routers may also have limitations and struggle to handle multiple devices, potentially causing lag.

It's also a good idea to , such as a mobile hotspot, to reduce the chances of downtime so that orders continue to flow even if your primary network goes down.

Atlas printers can connect to your network via wireless WiFi and LAN cable. We recommend using a LAN cable for the most reliable performance while keeping printers connected wirelessly to your backup WiFi.