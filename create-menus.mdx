...
* Modifier Groups – Customization options applied to products (e.g., "Choice of Sides," "Extra Toppings").
...
Products are individual menu items that diners can order, such as "Cheeseburger" or "Iced Latte". These are housed within Sections for better organization (e.g., "Burgers," "Drinks").

To add a product to a section, follow these steps:
...
Modifier groups act as containers for modifiers, which are individual customizations or add-ons that can be selected within a product. They allow diners to customize their orders by selecting additional options.
...
A Modifier is a single customizable component diners can select when ordering a product, that are housed inside Modifier groups. Modifiers can either be free (e.g., "No Onions") or paid (e.g., "Extra Cheese +$1").
